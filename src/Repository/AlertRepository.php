...
class AlertRepository extends ServiceEntityRepository
{
   ...
   /**
     * Returns alerts for a specific order
     * @param int $orderId
     * @param DashboardFiltersDTO|null $filters
     * @return Alert[]
     */
    public function findByOrderId(int $orderId, ?DashboardFiltersDTO $filters = null): array
    {
        $qb = $this->createQueryBuilder('a')
            ->andWhere('a.order = :orderId')
        ...
        if ($filters->choice !== null) {
            switch ($filters->choice) {
                case 'today':
                    ...
                    $qb->andWhere('a.timestamp BETWEEN :startDate AND :endDate')
